...
def main(path):
    ...
    if sim_init():
      ...
      print(arm_joints)
      set_movement(0, 0, 0)   
      a=[[10,10,10,10,10],[15,15,15,15,15],[30,30,30,30,30],[35,35,35,35,35],[40,40,40,40,40]]
      for i in range(5):
          for j in range(len(a)):
            #_,a=sim.simxGetJointPosition(client_ID, arm_joints[i] , sim.simx_opmode_blocking)
            #print(a)
            #sim.simxSetJointTargetPosition(client_ID, arm_joints[0],np.deg2rad(a[j][0]) , sim.simx_opmode_blocking)
            #sim.simxSetJointTargetPosition(client_ID, arm_joints[1],np.deg2rad(a[j][1]) , sim.simx_opmode_blocking)
            #sim.simxSetJointTargetPosition(client_ID, arm_joints[2],np.deg2rad(a[j][2]) , sim.simx_opmode_blocking)
            #sim.simxSetJointTargetPosition(client_ID, arm_joints[3],np.deg2rad(a[j][3]) , sim.simx_opmode_blocking)
            sim.simxSetJointTargetPosition(client_ID, arm_joints[i],np.deg2rad(a[j][i]) , sim.simx_opmode_blocking)
            time.sleep(0.5)
    time.sleep(1.0)
    sim_shutdown()
    time.sleep(1.0)
